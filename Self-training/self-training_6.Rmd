---
title: "Self-training classification round 6"
author: "Siri Frisli"
output:
  html_notebook:
    toc: yes
...
```{r}
covid <- readRDS("E:/Data/Training samples/misinformation_class_5.RDS")
...
```{r}
418607/(table(train$label)[1] * 2)
418607/(table(train$label)[2] * 2)
```

```{r}
train <- train |>
  mutate(case_wts = ifelse(label == "misinfo", 3.012775 , 0.5994916 ),
...
cm_lr$byClass["F1"] # 0.9902735  
cm_lr$byClass["Precision"] # 0.9994818  
cm_lr$byClass["Recall"] # 0.9812334  
```
...
```{r}
# saveRDS(lr_final_fit, "~/SVM_ST/log_reg_round6.RDS")
```
...
covid_predicted |>
  count(label) # mis = 91566, not mis = 444604
```

```{r}
saveRDS(covid_predicted, "E:/Data/Training samples/misinformation_class_6.RDS")
```