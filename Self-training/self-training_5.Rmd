...
```{r}
covid <- readRDS("E:/Data/Training samples/misinformation_class_4.RDS")
...
```{r}
395955/(table(train$label)[1] * 2)
395955/(table(train$label)[2] * 2)
```

```{r}
train <- train |>
  mutate(case_wts = ifelse(label == "misinfo", 3.179851, 0.5932888),
...
cm_lr$byClass["F1"] # 0.9932329 
cm_lr$byClass["Precision"] # 0.9995503 
cm_lr$byClass["Recall"] # 0.9869949 
```
```{r}
lr_preds |>
...
```{r}
# saveRDS(lr_final_fit, "~/SVM_ST/log_reg_round5.RDS")
```
...
```{r}
saveRDS(covid_predicted, "E:/Data/Training samples/misinformation_class_5.RDS")
```