---
title: "Self-training classification round 5"
author: "Siri Frisli"
output:
  html_notebook:
    toc: yes
...
Loading the data
```{r}
covid <- readRDS("D:/Data/Training samples/misinformation_class_4.RDS")
...
cm_svm$byClass["F1"] # 0.882   
cm_svm$byClass["Precision"] # 0.9055441  
cm_svm$byClass["Recall"] # 0.8596491   
```
...
cm_svm <- confusionMatrix(table(new_test$label, model_svm$...202)) 
cm_svm$byClass["F1"] # 0.8884422 
cm_svm$byClass["Precision"] # 0.9075975 
cm_svm$byClass["Recall"] # 0.8700787 
```
...
covid_pred_df <- covid_pred_df |>
  filter(non.misinfo > 0.99 | misinfo > 0.98)
```

```{r}
covid_pred_df_label <- covid_pred_df |>
  mutate(label = case_when(
    misinfo > 0.98 ~ "misinfo",
...
```{r}
saveRDS(covid_predicted, "D:/Data/Training samples/misinformation_class_5.RDS")
```
