...
```{r}
covid <-readRDS("E:/Data/covid_links_sample.RDS")
covid <- covid |>
  rename(tweet = text) |>
...
cm_svm_test$byClass["F1"] # 0.9083527   
cm_svm_test$byClass["Precision"] # 0.90625   
cm_svm_test$byClass["Recall"] # 0.9104651   
```
...
```{r}
svmTune <- tune(svm, train.x = label~., data = new_train, kernel='radial',
                ranges=list(cost=10^(-3:3), gamma=seq(0, 10, 1)))
```
...
cm_lr$byClass["F1"] # 0.8865979  
cm_lr$byClass["Precision"] # 0.8958333 
cm_lr$byClass["Recall"] # 0.877551 
```
