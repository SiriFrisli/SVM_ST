...
```{r}
# covid <- readRDS("E:/Data/manual_links_sample.RDS")
```
...
svm_tune$best.parameters$gamma # 0.001
svm_tune$best.parameters$cost # 4.5
```
...
cm_svm$byClass["F1"] # 0.8888889  
cm_svm$byClass["Precision"] # 0.9183673  
cm_svm$byClass["Recall"] # 0.861244  
```
```{r}
new_test |>
  bind_cols(predict(svm_model, new_test)) |>
...
cm_lr$byClass["F1"] # 0.8502415  
cm_lr$byClass["Precision"] # 0.8979592  
cm_lr$byClass["Recall"] # 0.8073394  
```
...
cm_bt$byClass["F1"] # 0.8461538  
cm_bt$byClass["Precision"] # 0.8979592  
