...
covid_os <- ovun.sample(label~., data = covid, method = "over", p = 0.2, seed = 1234)$data
...
svm_tune$best.parameters$cost # 0.6

set.seed(1234)
svm_tune_2 <- tune.svm(x = label ~., 
                       data = new_train,
                       gamma = seq(0.01,0.03, by = 0.005), 
                       cost = seq(0.5,0.7, by = 0.05), 
                       kernel = "radial")
