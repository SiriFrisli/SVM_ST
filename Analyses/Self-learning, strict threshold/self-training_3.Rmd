---
title: "Self-training classification round 3"
author: "Siri Frisli"
output:
  html_notebook:
    toc: yes
...
Loading the data
```{r}
covid <- readRDS("D:/Data/Training samples/misinformation_class_2.RDS")
...
cm_svm$byClass["F1"] # 0.8369906   
cm_svm$byClass["Precision"] # 0.8317757   
cm_svm$byClass["Recall"] # 0.8422713   
```
...
cm_svm <- confusionMatrix(table(new_test$label, model_svm$...202)) 
cm_svm$byClass["F1"] # 0.8352   
cm_svm$byClass["Precision"] # 0.8130841   
cm_svm$byClass["Recall"] # 0.8585526   
```
...
```{r}
saveRDS(covid_predicted, "D:/Data/Training samples/misinformation_class_3.RDS")
```